...
exports.renderTex = exports.convertTask = void 0;
const fs = require("fs");
...
function convertTask(taskFile, fileOut) {
    var _a;
    const langCode = (_a = util_1.parseLanguageCodeFromTaskPath(taskFile)) !== null && _a !== void 0 ? _a : codes.defaultLanguageCode();
    const textMd = util_1.readFileSyncStrippingBom(taskFile);
    ...
    const linealizedTokens = _.flatMap(tokens, t => {
        var _a;
        if (t.type === "inline") {
            return (_a = t.children) !== null && _a !== void 0 ? _a : [];
        }
        else {
            return [t];
        }
    ...
    const fileOutBrochure = util_1.siblingWithExtension(fileOut, "_brochure.tex");
...
function renderTex(linealizedTokens, langCode, metadata, filepath, standalone) {
    var _a;
    const license = patterns.genLicense(metadata);
    const skip = () => "";
    let _currentToken;
    let _currentSection = "prologue";
    function warn(msg) {
    ...
    function defaultRendererState() {
        return {
            ...
            currentTableCell: undefined,
            currentTableRowIndex: -1,
            currentTableColumnIndex: -1,
            validMultirows: [],
            lastRowTypeInThisTable: undefined,
    ...
    class RendererEnv {
        ...
        setState(newPartialState) {
            const newState = Object.assign(Object.assign({}, this.state()), newPartialState);
            this.stateStack[this.stateStack.length - 1] = newState;
            return newState;
        }
        pushState(newPartialState) {
            const newState = Object.assign(Object.assign({}, this.state()), newPartialState);
    ...
    function sectionCommandsForHeadingToken(t) {
    ...
    const expand = {
        "header": (tokens, idx, env) => {
            ...
            const ageCatTitles = Object.keys(ageCategories);
            ...
            const ageCatValueCells = ageCatTitles.map(c => {
                ...
                const catValue = metadata.ages[catFieldName] || "--";
                return catValue;
            }).join(" & ");
            ...
            function catToRow(catName) {
                ...
                return `${catChecked} ${util_1.texEscapeChars(catName)}`;
            }
            ...
            const keywordsStr = keywords.length === 0 ? "—" : keywords.map(util_1.texEscapeChars).join(", ");
            function multicolumn(n, contents) {
...
{\\footnotesize\\begin{tabularx}{\\columnwidth}{ | *{6}{ >{\\centering\\arraybackslash}X | } }
  ...
  ${ageCatValueCells} \\\\
  \\hline
  ${multicolumn(6, `\\textit{Answer Type:} ${util_1.texEscapeChars(metadata.answer_type)}`)} \\\\
  \\hline
...
\\renewcommand{\\tabularxcolumn}[1]{>{}m{#1}}\n`;
    ...
    function closeLineIfNeeded(env) {
    ...
    function nonExpandingAlignment(possiblyExpandingAlignment) {
    ...
    function openCellPushingState(type, token, env) {
        var _a;
        ...
        const align = nonExpandingAlignment((_a = state.currentTable) === null || _a === void 0 ? void 0 : _a.cellAlignmentChars[colIndex]);
    ...
    function closeCellPoppingState(env) {
        var _a, _b;
        const cellState = env.popState();
        env.setState({ hasCellOnThisLine: true });
        return (_b = (_a = cellState.currentTableCell) === null || _a === void 0 ? void 0 : _a.closeWith) !== null && _b !== void 0 ? _b : "";
    }
    function breakIfInTableCell(env) {
    ...
    function isSurrounded(tokens, idx, distance, item, itemClose) {
        let itemOpen;
        if (lodash_1.isUndefined(itemClose)) {
            itemOpen = `${item}_open`;
    ...
    const rules = {
        ...
        "text": (tokens, idx, env) => {
            ...
            text = util_1.texEscapeChars(text);
            const state = env.state();
            if (!state.isInHeading && !state.disableMathify) {
                text = util_1.texMathify(text);
            }
            return text;
        },
        "image": (tokens, idx, env) => {
            ...
            if (title && (match = patterns.imageOptions.exec(title))) {
                ...
                if (value = match.groups.width_abs) {
                    const f = roundTenth(parseFloat(value) * util_1.HtmlToTexPixelRatio);
                    width = `${f}px`;
            ...
            function useRaisebox(ignoreHeight) {
            ...
            if (placement === "unspecified" || isInTable) {
                if (isSurrounded(tokens, idx, 1, "paragraph")) {
                    ...
                    try {
                        ...
                        ignoreHeight = parseInt((_a = width === null || width === void 0 ? void 0 : width.replace(/px/, "")) !== null && _a !== void 0 ? _a : "0") < 30;
                    }
                    catch (_b) { }
        ...
        "math_inline": (tokens, idx, env) => {
            ...
            return '${' + util_1.texMath(tokens[idx].content) + '}$';
        },
        ...
        "math_block_eqno": (tokens, idx, env) => {
            return '$$' + util_1.texMath(tokens[idx].content) + '$$'; // TODO add eqno?
        },
        ...
        "table_open": (tokens, idx, env) => {
            const t = tokens[idx];
            function columnSpec(alignString, hresize) {
                switch (alignString) {
                    case "":
            ...
            if (rule) {
                if (r = rule(tokens, idx, env)) {
                    if (lodash_1.isString(r)) {
                        ...
                        if (lodash_1.isUndefined(secParts)) {
                            secParts = [r];
...
\\renewcommand*{\\FBguillspace}{\\hskip .4\\fontdimen2\\font plus .1\\fontdimen3\\font minus .3\\fontdimen4\\font \\relax}`,
    ...
    const babel = (_a = babels[langCode]) !== null && _a !== void 0 ? _a : babels.eng;
    function difficultyIndex(ageCat) {
    ...
    function normalizeAuthorName(fullName) {
        ...
        if (parts.length === 1) {
            ...
            const split = [asciify(parts[parts.length - 1]), asciify(parts[0][0]).toUpperCase()];
    ...
    function authorDefs() {
        ...
        metadata.contributors.forEach((contribLine) => {
            ...
            if (match) {
                ...
                const lowercaseCountryCode = (_b = (_a = codes.countryCodeByCountryName[match.groups.country]) === null || _a === void 0 ? void 0 : _a.toLowerCase()) !== null && _b !== void 0 ? _b : "aa";
    ...
    function sectionTexFor(secName) {
        var _a;
        return ((_a = sectionStrs[secName]) !== null && _a !== void 0 ? _a : ["TODO"]).join("");
    }
    if (!standalone) {
...
\\lfoot{\\scriptsize ${util_1.texEscapeChars(license.shortCopyright())}}
\\cfoot{\\scriptsize\\itshape ${util_1.texEscapeChars(metadata.id)} ${util_1.texEscapeChars(metadata.title)}}
...
//# sourceMappingURL=convert_tex.js.map